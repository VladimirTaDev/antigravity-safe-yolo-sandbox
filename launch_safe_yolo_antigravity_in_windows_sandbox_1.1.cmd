...
if not exist "%SystemRoot%\System32\WindowsSandbox.exe" (
    echo ERROR: Windows Sandbox is not enabled or not supported on this OS.
    echo Please enable it via "Turn Windows features on or off".
    pause
    exit /b 1
)
...
:: Check if agy is installed on the host
if not exist "%AGY_DIR%\bin\agy.exe" (
    echo ERROR: Google Antigravity ^(agy^) is not installed on this host machine.
    echo Please install it first, as the sandbox maps the executable read-only.
    pause
    exit /b 1
)
...
>>"%TMP_DIR%\extract_cred.ps1" echo         return Convert.ToBase64String(blob);
>>"%TMP_DIR%\extract_cred.ps1" echo     }
>>"%TMP_DIR%\extract_cred.ps1" echo }
>>"%TMP_DIR%\extract_cred.ps1" echo '@
...
>>"%TMP_DIR%\extract_cred.ps1" echo }
>>"%TMP_DIR%\extract_cred.ps1" echo [System.IO.File]::WriteAllText("$PSScriptRoot\keyring_token.b64", $token)
...
>>"%TMP_DIR%\inject_cred.ps1" echo         byte[] blob = Convert.FromBase64String(secret);
...
>>"%TMP_DIR%\inject_cred.ps1" echo }
>>"%TMP_DIR%\inject_cred.ps1" echo '@
>>"%TMP_DIR%\inject_cred.ps1" echo $tokenFile = Join-Path $PSScriptRoot "keyring_token.b64"
>>"%TMP_DIR%\inject_cred.ps1" echo $token = (Get-Content $tokenFile -Raw).Trim()
>>"%TMP_DIR%\inject_cred.ps1" echo Remove-Item -Force $tokenFile -ErrorAction SilentlyContinue
...
if not exist "%TMP_DIR%\keyring_token.b64" (
...
:: Close processes first to release SQLite vSMB locks
>>"%TMP_DIR%\Close_Sandbox_Preserving_agy_history_and_settings.cmd" echo echo Closing agy instances to release SQLite locks...
>>"%TMP_DIR%\Close_Sandbox_Preserving_agy_history_and_settings.cmd" echo taskkill /F /IM agy.exe /T 2^^^>nul
>>"%TMP_DIR%\Close_Sandbox_Preserving_agy_history_and_settings.cmd" echo taskkill /F /IM pwsh.exe /T 2^^^>nul
...
>>"%TMP_DIR%\sandbox-init.cmd" echo echo [1/8] Installing Node.js for MCP Servers...
>>"%TMP_DIR%\sandbox-init.cmd" echo curl -fsSL https://nodejs.org/dist/v20.15.0/node-v20.15.0-x64.msi -o %%TEMP%%\nodejs.msi
>>"%TMP_DIR%\sandbox-init.cmd" echo powershell -NoProfile -Command "if ((Get-FileHash $env:TEMP\nodejs.msi).Hash -ne '0945B75AF2EB884790064D90DC2E05CB3443C196C2FF546D7354B81A0721F882') { Write-Error 'Node.js hash mismatch'; exit 1 }"
>>"%TMP_DIR%\sandbox-init.cmd" echo if %%errorlevel%% neq 0 ( pause ^& exit /b 1 )
>>"%TMP_DIR%\sandbox-init.cmd" echo start /wait msiexec /i %%TEMP%%\nodejs.msi /quiet /norestart
>>"%TMP_DIR%\sandbox-init.cmd" echo del %%TEMP%%\nodejs.msi
>>"%TMP_DIR%\sandbox-init.cmd" echo.
>>"%TMP_DIR%\sandbox-init.cmd" echo echo [2/8] Copying .gemini configuration locally to avoid SQLite vSMB locks...
...
>>"%TMP_DIR%\sandbox-init.cmd" echo.
>>"%TMP_DIR%\sandbox-init.cmd" echo echo [3/8] Injecting authentication into Credential Manager...
...
>>"%TMP_DIR%\sandbox-init.cmd" echo.
>>"%TMP_DIR%\sandbox-init.cmd" echo echo [4/8] Adding agy to PATH...
>>"%TMP_DIR%\sandbox-init.cmd" echo set "PATH=%%LOCALAPPDATA%%\agy\bin;%%PATH%%"
>>"%TMP_DIR%\sandbox-init.cmd" echo powershell -NoProfile -Command "[Environment]::SetEnvironmentVariable('Path', [Environment]::GetEnvironmentVariable('Path', 'User') + ';%%LOCALAPPDATA%%\agy\bin', 'User')"
>>"%TMP_DIR%\sandbox-init.cmd" echo.
>>"%TMP_DIR%\sandbox-init.cmd" echo echo [5/8] Installing PowerShell 7...
...
>>"%TMP_DIR%\sandbox-init.cmd" echo powershell -NoProfile -Command "if ((Get-FileHash $env:TEMP\pwsh.msi).Hash -ne '57342B7B9ACA6C53B27EE40A307E8FC5DC28B97218BFD016C7941CE25621F79B') { Write-Error 'PWSH hash mismatch'; exit 1 }"
>>"%TMP_DIR%\sandbox-init.cmd" echo if %%errorlevel%% neq 0 ( pause ^& exit /b 1 )
...
>>"%TMP_DIR%\sandbox-init.cmd" echo.
>>"%TMP_DIR%\sandbox-init.cmd" echo echo [6/8] Installing Windows Terminal...
...
>>"%TMP_DIR%\sandbox-init.cmd" echo powershell -NoProfile -Command "if ((Get-FileHash $env:TEMP\wt.zip).Hash -ne 'F480F65A7874E7055C51EC67E0E4F98011A4EF63DD43C6B882458DAE36A83286') { Write-Error 'WT hash mismatch'; exit 1 }"
>>"%TMP_DIR%\sandbox-init.cmd" echo if %%errorlevel%% neq 0 ( pause ^& exit /b 1 )
...
>>"%TMP_DIR%\sandbox-init.cmd" echo.
>>"%TMP_DIR%\sandbox-init.cmd" echo echo [7/8] Launching YOLO mode in Windows Terminal...
...
>>"%TMP_DIR%\sandbox-init.cmd" echo.
>>"%TMP_DIR%\sandbox-init.cmd" echo echo [8/8] Creating Desktop Shortcut and Cleanup...
...
:: Wait briefly to ensure Sandbox reads the file, then delete it from host
timeout /t 5 >nul
del "%TEMP%\yolo-sandbox.wsb"
exit
